...
基于 uGUI 的 FairyGUI Runtime SDK。继续用 **FairyGUI Editor** 编辑 UI，用 NanamiUI 把 FairyGUI 工程转换成 **uGUI prefab + C# 代码**，并尽量还原 FairyGUI 的渲染、动效与交互表现。
...
- `Runtime/`：运行时 SDK，命名空间 `NanamiUI`（`Text`/`Button`/`ComboBox`/`Slider`/`ScrollPane`/`MovieClip`/`Transition`/`Gear*` …）。
- `Editor/`：通用转换器与工程 postprocess，命名空间 `NanamiUI.Editor`。
- `Example/`：Basics demo 的运行时胶水。

## 使用（Unity 菜单 `Tools/NanamiUI`）
...
3. **Run PlayMode Tests** — 跑全部 PlayMode parity 测试，结果写入 `Temp/NanamiUIPlayModeResults.txt`。
...
以下按设计**不实现**（用 Unity 原生替代或不需要）：resource URL / 动态异步资源加载、FairyGUI runtime API 签名一致性、GList 虚拟化/动态增删/selection、GTree、tooltips 浮层、blur 滤镜、跨组件 Controller page-action。详见仓库根 `AGENTS.md`「发布范围与取舍」，实现状态见 `Docs/BasicsImplementationStatus.md`。
